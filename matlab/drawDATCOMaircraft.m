...
%Reading vertical tail geometry
    ...
    if vaflstat==1
        ...
        %Extracting vertical tail airfoil geometry
        if exist('VAFLNX','var')
        prop={'XCORD(1)','MEAN(1)','THICK(1)','YUPPER(1)','YLOWER(1)'};
        for propcount=1:numel(prop)
            for count=1:numel(vaflstring)
...
%                    XH+HCHRDR];
            xhtp=[XH,...
                (XH+(HCHSTAT*HCHRDR)+(HSSPN*tand(HSAVSI))-...
                (HCHSTAT*HCHRDTP)),(XH+(HCHSTAT*HCHRDR)+...
                (HSSPNOP*tand(HSAVSO))-(HCHSTAT*HCHRDTP)),...
                (XH+(HCHSTAT*HCHRDR)+(HSSPNOP*tand(HSAVSO))+...
                ((1-HCHSTAT)*HCHRDTP)),XH+HCHRDR];
%             yhtp=[0,HSSPN,HSSPN,0];
            yhtp=[0,HSSPN-HSSPNOP,HSSPN,HSSPN,0];
%             zhtp=[ZH,...
...
%                    (ZH-(HAT*HCHRDR))];
            zhtpf=[(ZH+(HAT*HCHRDR)),...
                   (ZH+(HSSPN*tand(HDHDADI))+(HAT*HCHRDTP)),...
                   (ZH+(HSSPN*tand(HDHDADO))+(HAT*HCHRDTP)),...
                   (ZH+(HSSPN*tand(HDHDADO))-(HAT*HCHRDTP)),...
                   (ZH-(HAT*HCHRDR))];
        end
        %Calculating LE and TE line for double cranked planform
...
%Drawing aircraft's topview
    ...
    if ZW<testval
        ...
        wtv_left=fill([xwing,xwing(end:-1:1)],-[ywing,ywing(end:-1:1)],...
                      'w','Parent',topviewaxis,'linewidth',line_width);
...
%Drawing aircraft's sideview
    ...
    if drawhtp==1
        ...
        %Drawing inboard horizontal tail
        if (HDHDADI>=0)
...
%Drawing wing's airfoil
    ...
    if drawwingafl==1
        grid on;
        waflu=plot(WAFLX,WAFLYU,'Parent',wingaflaxis,'linewidth',line_width);
        grid on;
        wafll=plot(WAFLX,WAFLYL,'Parent',wingaflaxis,'linewidth',line_width);
...
%Drawing horizontal tail's airfoil
    ...
    if haflstat==1
    if drawhtpafl==1
        haflu=plot(HAFLX,HAFLYU,'Parent',htpaflaxis,'linewidth',line_width);
        grid on;
        hafll=plot(HAFLX,HAFLYL,'Parent',htpaflaxis,'linewidth',line_width);
        grid on;
    end
    end
    %Equalizing axis scale
...
%Drawing vertical tail's airfoil
    ...
    if vaflstat==1
    if drawvtpafl==1
        vaflu=plot(VAFLX,VAFLYU,'Parent',vtpaflaxis,'linewidth',line_width);
        grid on;
        vafll=plot(VAFLX,VAFLYL,'Parent',vtpaflaxis,'linewidth',line_width);
        grid on;
    end
    end
    %Equalizing axis scale
